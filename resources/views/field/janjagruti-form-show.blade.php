<x-admin.admin-layout>
    <x-slot name="title"> {{ $user->nodal_person_name }} - जनजागृती स्पर्धा</x-slot>
                ...
                <div class="col-12 px-0">
                    <div class="card">
                        ...
                        <div class="card-body p-2">
                            <div class="row">
                                <div class="col-12">
                                    @foreach ($user->contestTwo->groupBy('department_id') as $contests)
                                                    ...
                                                    @foreach ($contests as $contest)
                                                        <div class="col-12 top-brdr mb-2">
                                                            ...
                                                            <div class="row">
                                                                <div class="col-4">
                                                                    @if ($contest->picture)
                                                                        <span> <a href="{{ asset($contest->picture) }}" class="btn btn-light btn-sm px-2 py-1" target="_blank">View Photo</a> </span> &nbsp;&nbsp;&nbsp;
                                                                    @endif
                                                                </div>
                                                                <div class="col-4">
                                                                    @if ($contest->video)
                                                                        <span> <a href="{{ asset($contest->video) }}" class="btn btn-light btn-sm px-2 py-1" target="_blank">View Video</a> </span> <br>
                                                                    @endif
                                                                </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <input type="hidden" {{ $user->field_submitted_user_id ? 'disabled' : '' }} name="user_contest_two_id" id="user_contest_two_id" value="">
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function() {
                $("#editForm").submit(function(e) {
                    ...
                    var url = "{{ route('field.janjagruti_form.update', ":model_id") }}";
                    //
                    $.ajax({
                        url: url.replace(':model_id', model_id),
                        type: 'POST',
        ...
        <script>
            $(".final-submit").click(function(e) {
                e.preventDefault();
                var user_id = $(this).attr("data-userid");
                var url = "{{ route('field.janjagruti_form.final-submit', ':model_id') }}";
